...
static void testA(void *param);
static void testB(void *param);
static void testC(void *param);
static void vCpuWork(void *pvParameters);

const portTickType test_delay = 1 / portTICK_RATE_MS;
const portTickType test_freq = 5 / portTICK_RATE_MS;

portTickType xLastWakeTimeA;
portTickType xLastWakeTimeB;
portTickType xLastWakeTimeC;
...
int main()
{
	...
	xTaskCreate( testA, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
	xTaskCreate( testB, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
	xTaskCreate( testC, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
...
static void testA(void *param)
{
	...
	for( ;; )
	{
		...
		vTaskDelayUntil( &xLastWakeTimeA, test_freq );
...
static void testB(void *param)
{
	...
	{
		// Wait for the next cycle.
		vTaskDelayUntil( &xLastWakeTimeB, test_freq );
...
static void testC(void *param)
{
	...
	{
		// Wait for the next cycle.
		vTaskDelayUntil( &xLastWakeTimeC, test_freq );
